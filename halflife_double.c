...
#define NUMPARS   6          /*number of parameters to fit*/
#define NUMFUNC   3          /*number of separate functions in fit profile excl. bkgnd*/
#define MITER     70         /*max. number of iterations for fitter routine*/
#define CHLEN     120        /*character length of filename arrays*/
#define CHFLEN    32         /*character length of function name arrays*/
#define CHPLEN    33         /*character length of parameter name arrays*/
...
/*#define P_OPT_END "-world 0 0 2050 1500 -pexec \"xaxis tick major 500\" -pexec \"xaxis tick minor ticks 5\""*/
...
/* Carl Wheldon March 2023 */

/* To compile:
gcc halflife_double.c -Wall -pedantic -o halflife_double -lm -O2
*/

/*%%%% A program to fit back-to-back curves of a Gaussian convoluted with
    an exponential decay and a prompt Gaussian component %%%%*/
...
int main(int argc, char *argv[])
{
    ...
    strncpy(par_nm[4], "Left-to-right ratio", 19);
    strncpy(par_nm[5], "Relative Prompt Gaussian scaling", 32);
    strncpy(par_nm[NUMPARS], "Background level (not fitted)", 29);
    
    /*copy function names to function components in eval()*/
    strncpy(func_nm[0], "Gaus-exp conv right-hand side", 29);
    strncpy(func_nm[1], "Gaus-exp conv left-hand side", 28);
    strncpy(func_nm[2], "Prompt Gauss component", 22);
    ...
    /*initialise relative scaling parameters to 1.0*/
    init[4] = 1.0;
    init[5] = 1.0;
    /*initialise background parameter (constant offset) to zero*/
    init[NUMPARS] = 0.0;
    ...
    printf("\n\t**** Welcome to program %s ****\nThis fits"
    	   " back-to-back curvies comprising a Gaussian convoluted with an"
           " exponential decay and prompt Gaussian component.\n"
	...
	else if (md == 4 && flginit)
	{
	    ...
	    {
                /* t_1/2 < 0 need to invert left-to-right ratio (pars[4])
                    but also correct scaling factor pars[3] accordingly*/
                if (ft.pars[0] < 0.0)
                {
                    printf("%sNegative half-life --> swapping left and right and refitting%s\n",
                        clr[3],clr[0]);
                    ft.pars[0] = abs(ft.pars[0]);
                    ft.pars[3] *= ft.pars[4];
                    ft.pars[5] /= ft.pars[4];
                    ft.pars[4] = 1.0/ft.pars[4];
                    /*since good fit achieved, up-date init[0] to prevent this*/
                    init[0] = ft.pars[0];
                    if ( (r = fitter(MITER, &chisq, 1)) != 0 ) break;
                }
                /*write output file*/
...
/*    	*fit = pars[3]*( gauss_exp(x - pars[2], pars[0], pars[1]) +
          pars[4]*gauss_exp(x-pars[2], -1.0*pars[0], pars[1]) +
          pars[5]*gauss(x - pars[2], pars[1]) );*/
    	
        /*right-hand gauss-exp convolution*/
        *fit = pars[3]*( gauss_exp(x - pars[2], pars[0], pars[1]));
        if (mode == 2) return 0;
	
        /*left-hand gauss-exp convolution*/
        ft1 = pars[3]*pars[4]*gauss_exp(x-pars[2], -1.0*pars[0], pars[1]);
        if (mode == 3)
        {
            *fit = ft1;
            return 0;
        ...
        /*prompt gaussian component*/
        ft2 = pars[3]*pars[5]*gauss(x - pars[2], pars[1]);
        if (mode == 4)
        {
            *fit = ft2;
            return 0;
    ...
    if (mode != 3)
    {
        ...
        for (j = 0; j < (NUMFUNC+1); j++)
        {
    	    ...
    	    printf("    Function %d (%s),%*s area =%12.1f cnts\n",
                    j,func_nm[j],CHFLEN-(int)strlen(func_nm[j]),"",sum);
            /*after the second Guass-exp function, print the sum of both*/
            if (j == 1)
            {
                fprintf(fout, "#Sum of areas of functions %d and %d,%*s area =%12.3f cnts\n",
                    j-1,j,16,"",sumo);
   	        printf("    %sSum of areas of functions %d and %d,%*s area =%12.1f cnts%s\n",
                    clr[2],j-1,j,12,"",sumo,clr[0]);
            }
         }
    }
    else if (mode == 3)
